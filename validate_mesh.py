...
async def test_endpoint(client: httpx.AsyncClient, name: str, path: str, method: str = "GET", params: Dict = None, data: Dict = None):
    start = time.time()
    try:
        if method == "GET":
...
async def run_validation():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        results.append(await test_endpoint(client, "Full-Stack Audit Agent", "/test/run", "POST", params={"task": "Full Stack Integrity Audit"}))
        ...
        results.append(await test_endpoint(client, "API Hub Bridge", "/api/test-connection", params={"provider": "local_mythos"}))
        
        # 5. Strategic Intelligence (Real Scraping Test)
        results.append(await test_endpoint(client, "Strategic Intel Engine", "/intel", params={"url": "https://www.google.com"}))

        # 6. Self-Evolving Core (Run Evolved Test)
        results.append(await test_endpoint(client, "Self-Evolving Test", "/evolve/run-test", method="POST", params={"layer": "frontend", "task": "Full Stack Integrity Audit"}))
    ...
    for r in results:
        ...
        total_score += score
        print(f"{r['name']:<25} | {r['status']:<10} | {r['latency']:>7.2f}ms | {score:.1f}/{max_score_per_test:.1f}")
